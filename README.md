...
我在做搜索框的时候有suggest的需求，首先第一个想法肯定是先找找有没有成熟的第三方框架或者组件，[gmu](http://gmu.baidu.com/)和[NovaUI](http://75team.github.io/novaUI/)是我首先考虑两个库。它们已经实现了我的需求，但是发现依赖了库里很多公用文件，这是我不想看到的。
...
最简单用法实例化一个Suggest对象就可以了，实用默认配置来实现功能。

## 定制

对于具体的业务需求，默认配置一般满足不了需求，需要进行相应的配置。对Suggest实例化时传入一个对象来覆盖默认配置。一般而言，需要配置的为条目模板和数据接口相关的一些属性信息。
...
为实现这样的功能，我们要么是用户修改回调函数的相关代码，指定需要便利的数组，要么就是组件提供一个接口，让用户可以对返回的数据进行相应操作。但是，这些做法都丧失了组件的灵活性。

这里的组件内容实用了一个getPathData的函数，根据path来查找数组或者对象中指定path的值。

如，它可以通过path:'a.1'来对data={a:[1,2],b:'ha'}进行操作，获取想要的2。
